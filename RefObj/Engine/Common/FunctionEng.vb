...
Namespace Common
    Public Class FunctionEng
        ...
        Public Shared Function GetShopConfig(ByVal ParaName As String) As String
            ...
            Dim lnq As New LinqDB.TABLE.CfSettingLinqDB
        ...
        Public Shared Function GetShopDataTable(ByVal sql As String) As DataTable
            ...
            dt = LinqDB.ConnectDB.SqlDB.ExecuteTable(sql)
            Return dt
        End Function

        Public Shared Function GetShopDataTable(ByVal sql As String, trans As SqlTransaction) As DataTable
            Dim dt As New DataTable
            dt = LinqDB.ConnectDB.SqlDB.ExecuteTable(sql, trans)
            Return dt
        End Function

        Public Shared Function ExecuteShopNonQuery(ByVal sql As String) As Boolean
            Dim ret As Boolean = False
            Dim shTrans As New LinqDB.ConnectDB.TransactionDB
            Try
                LinqDB.ConnectDB.SqlDB.ExecuteNonQuery(sql, shTrans.Trans)
        ...
        Public Shared Function ExecuteShopNonQuery(ByVal sql As String, ByVal shTrans As LinqDB.ConnectDB.TransactionDB) As Boolean
            Dim ret As Boolean = False
            Try
                ret = (LinqDB.ConnectDB.SqlDB.ExecuteNonQuery(sql, shTrans.Trans) > 0)
            Catch ex As Exception
        ...
        Public Shared Sub CreateLogFile(FileName As String, ByVal TextMsg As String)
            'Dim FILE_NAME As String = System.Windows.Forms.Application.StartupPath & "\" & DateTime.Now.ToString("yyyyyMMddHH") & ".sql"
        ...
        Public Shared Function CheckShopConnect() As Boolean
            Return LinqDB.ConnectDB.SqlDB.ChkConnection()
        End Function
        ...
        '    Return LinqDB.ConnectDB.SqlDB.ChkConnection(ServerName, UserID, Passwd, DBName)
        'End Function
        ...
        Public Shared Function GetShopConfig(ByVal ParaName As String, ByVal ShTrans As LinqDB.ConnectDB.TransactionDB) As String
            Dim ret As Boolean = False
            Dim lnq As New LinqDB.TABLE.CfSettingLinqDB
        ...
        Public Shared Function GetShopItemPara(ByVal ItemID As Integer, ByVal ShTrans As LinqDB.ConnectDB.TransactionDB) As LinqDB.TABLE.MsServiceLinqDB
            Dim lnq As New LinqDB.TABLE.MsServiceLinqDB
        ...
        Public Shared Function cStrToDateTime2(ByVal StrDate As String) As DateTime 'Convert วันที่จาก dd/MM/yyyy ปี พ.ศ. เป็น DateTime
            Dim ret As New Date(1, 1, 1)
            If StrDate.Trim <> "" Then
                Dim vDate() As String = Split(StrDate, "/")
                ret = New Date(Convert.ToInt16(vDate(2)) - 543, vDate(1), vDate(0))
            End If
            Return ret
        End Function

        Public Shared Function cStrToDateTime3(ByVal StrDate As String) As DateTime 'Convert วันที่จาก yyyyMMdd ปี พ.ศ. เป็น DateTime
            Dim ret As New Date(1, 1, 1)
            If StrDate.Trim <> "" Then
                Dim vYear As Integer = Left(StrDate, 4)
                Dim vMonth As Integer = StrDate.Substring(4, 2)
                Dim vDate As Integer = Right(StrDate, 2)
                ret = New Date(vYear - (543 * 2), vMonth, vDate)
            End If
            Return ret
        End Function
        ...
        Public Shared Function GetItem(ByVal cID As Long, ByVal shTrans As LinqDB.ConnectDB.TransactionDB) As LinqDB.TABLE.MsServiceLinqDB
            Dim lnq As New LinqDB.TABLE.MsServiceLinqDB
            lnq.GetDataByPK(cID, shTrans.Trans)
        ...
        Public Shared Function GetDateNowFromDB() As DateTime
            Dim ret As DateTime
            Dim dt As DataTable = LinqDB.ConnectDB.SqlDB.ExecuteTable("select getdate() date_now")
            If dt.Rows.Count > 0 Then
...
#Region " Encrypt/Decrypt "
        ...
        Public Shared Function EnCripPwd(ByVal passString As String) As String
            Return LinqDB.ConnectDB.SqlDB.EnCripPwd(passString)
        End Function

        Public Shared Function DeCripPwd(ByVal passString As String) As String
            Return LinqDB.ConnectDB.SqlDB.DeCripPwd(passString)
        End Function
#End Region
    End Class
End Namespace
