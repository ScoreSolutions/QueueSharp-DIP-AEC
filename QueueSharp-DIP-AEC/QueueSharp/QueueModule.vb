...
#End Region
    ...
    Function CheckQueueStatus(ByVal TbRegisterQueueID As Long, ByVal Status As Integer, Optional ByVal shTrans As SqlTransaction = Nothing) As Boolean
        Dim sql As String = ""
        Dim dt As New DataTable
        '@QueueNo as int,
        '@CustomerID as varchar,
        '@Status as int
        sql = "exec SP_CheckQueueStatus '" & TbRegisterQueueID & "'," & Status
    ...
    Public Function UpdateQueueStatus(ByVal Status As Integer, TbRegisterQueueID As Long, TbCounterQueueID As Long, MsServiceID As Long, Flag As String, Optional ByVal _Trans As SqlTransaction = Nothing) As Boolean
        Dim ret As Boolean = False
        Dim sql As String = ""
        ...
        Select Case Status
            Case 1
                ...
                sql += " set status = '" & Status & "' ,ms_counter_id = 0,ms_user_id = 0"
                sql += " ,call_time = null,start_time = Null,call_auto_force='0'"
                sql += " where tb_register_queue_id='" & TbRegisterQueueID & "' and status in (2,4,6,8)"
            Case 2
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "', start_time = '" & DateNowStr & "',[ms_user_id] = '" & myUser.user_id & "',ms_counter_id = '" & myUser.counter_id & "'"
                sql += " where tb_register_queue_id='" & TbRegisterQueueID & "' and status = 4"
            Case 3
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "',end_time = '" & DateNowStr & "',[ms_user_id] = '" & myUser.user_id & "',ms_counter_id = '" & myUser.counter_id & "'"
                sql += " where id='" & TbCounterQueueID & "'"
            Case 4
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "',call_time = '" & DateNowStr & "',[ms_user_id] = '" & myUser.user_id & "',ms_counter_id = '" & myUser.counter_id & "' "
                sql += " where tb_register_queue_id='" & TbRegisterQueueID & "' and status in (1)"
            Case 5
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "',end_time = '" & DateNowStr & "',[ms_user_id] = '" & myUser.user_id & "',ms_counter_id = '" & myUser.counter_id & "'"
                sql += " where tb_register_queue_id='" & TbRegisterQueueID & "'"
            Case 6
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "',tb_register_queue_hold_id=0,[user_id] = '" & myUser.user_id & "',counter_id = '" & myUser.counter_id & "'"
                sql += " where id='" & TbCounterQueueID & "' and status =6 "
            Case 8
                sql = "update TB_COUNTER_QUEUE "
                sql += " set status = '" & Status & "',start_time = '" & DateNowStr & "',[ms_user_id] = '" & myUser.user_id & "',hold = null "
                sql += " where tb_register_queue_id='" & TbRegisterQueueID & "' and status in (4)"
        End Select
        ret = executeSQL(sql, _Trans, True)
        ...
        If ret = True Then
            ...
            ElseIf Status = 2 Or Status = 4 Then
                InsertMainDisplay(TbRegisterQueueID, myUser.counter_id, DateNowStr, Status, _Trans)
            End If


            If TbCounterQueueID = 0 And TbRegisterQueueID > 0 Then
                Dim dt As DataTable = getDataTable("select id, ms_service_id from tb_counter_queue where tb_register_queue_id='" & TbRegisterQueueID & "'", _Trans)
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        InsertLog(dr("id"), myUser.user_id, myUser.counter_id, dr("ms_service_id"), Status, "", _Trans, vDateNow)
                    Next
                End If
                dt.Dispose()
            Else
                InsertLog(TbCounterQueueID, myUser.user_id, myUser.counter_id, MsServiceID, Status, Flag, _Trans, vDateNow)
            End If
        End If
    ...
    Public Function UpdateCallStatus(RegisterID As Long, ByVal DateNow As DateTime, ByVal shTrans As SqlTransaction) As Boolean
        ...
        Sql += " set status = " & Status & ",call_time = '" & vDateNow & "',ms_user_id = '" & UserID & "',ms_counter_id = '" & CounterID & "', "
        ...
        If ret = True Then
            ...
            If ret = True Then
                Dim lnq As New LinqDB.TABLE.TbCounterQueueLinqDB
                Dim qDt As DataTable = lnq.GetDataList("tb_register_queue_id='" & RegisterID & "'  and status = 1 ", "id", shTrans)
                If qDt.Rows.Count > 0 Then
                    For Each qDr As DataRow In qDt.Rows
                        InsertLog(qDr("id"), myUser.user_id, CounterID, qDr("ms_service_id"), Status, "", shTrans, DateNow)
                    Next
