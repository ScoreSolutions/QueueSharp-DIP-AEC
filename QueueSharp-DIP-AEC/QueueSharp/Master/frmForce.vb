            ...
            For i As Int32 = 0 To 1000
                ...
                If ST_Date <= dtpEnd.Value Then
                    ...
                    For x As Int32 = 0 To dtSlot.Rows.Count - 1
                        ...
                        'sql = "insert into TB_FORCE_SCHEDULE(id,force_date,slot,wait,start_time,end_time,"
                        'sql += " create_by,create_date,start_slot,end_slot) "
                        ' sql += " values(" & ID & ",'" & FixDate(ST_Date) & "'," & cbSlot.Text & "," & nudWait.Value & ",'" & FixDate(ST_Date) & " " & dtSlot.Rows(x).Item("ST").ToString & "','" & FixDate(ST_Date) & " " & dtSlot.Rows(x).Item("ET").ToString & "'," & myUser.user_id & ",getdate(),'" & FixDate(ST_Date) & " " & CBstart_time_h.Text & ":" & CBstart_time_m.Text & "','" & FixDate(ST_Date) & " " & CBend_time_h.Text & ":" & CBend_time_m.Text & "')"
                        'executeSQL(sql)

                        Dim lnq As New LinqDB.TABLE.TbForceScheduleLinqDB
                        lnq.FORCE_DATE = ST_Date
                        lnq.WAIT = nudWait.Value
                        lnq.START_TIME = ST_Date.ToString("yyyy-MM-dd", New Globalization.CultureInfo("th-TH")) & " " & dtSlot.Rows(x).Item("ST").ToString
                        lnq.END_TIME = ST_Date.ToString("yyyy-MM-dd", New Globalization.CultureInfo("th-TH")) & " " & dtSlot.Rows(x).Item("ET").ToString
                        lnq.SLOT = cbSlot.Text
                        lnq.START_SLOT = ST_Date.ToString("yyyy-MM-dd", New Globalization.CultureInfo("th-TH")) & " " & CBstart_time_h.Text & ":" & CBstart_time_m.Text
                        lnq.END_SLOT = ST_Date.ToString("yyyy-MM-dd", New Globalization.CultureInfo("th-TH")) & " " & CBend_time_h.Text & ":" & CBend_time_m.Text
