...
Public Class frmServiceCancel
        ...
        Select Case True
            Case RadioHold.Checked 'Hold
                If CheckHold(QueueNo, CustomerID) = False Then
                    UpdateQueueStatus(6, TbRegisterQueueID, TbCounterQueueID, MsServiceID, txtCancelAll.Text.Trim)
                End If
                ...
                If CountEnd = 1 Then
                    For i As Int32 = 0 To GridItem.Rows.Count - 1
                        If GridItem.Rows(i).Cells("cb").Value = True Then
                            ...
                            executeSQL(sql)
                            'InsertLog(QueueNo, CustomerID, myUser.user_id, GridItem.Rows(i).Cells("item_id").Value.ToString, myUser.counter_id, 5, Flag & " Remarks:" & txtCancelAll.Text, Nothing, vDateNow)
                        End If
                    Next
                ...
                Else
                    ...
                    For i As Int32 = 0 To GridItem.Rows.Count - 1
                        If GridItem.Rows(i).Cells("cb").Value = True Then
                            ...
                            End If
                            'InsertLog(QueueNo, CustomerID, myUser.user_id, GridItem.Rows(i).Cells("item_id").Value.ToString, myUser.counter_id, 5, "Cancel Service From frmEndByService Remarks:" & txtCancelAll.Text, Nothing, vDateNow)
    ...
    Private Sub frmServiceCancel_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        Dim dt As New DataTable
        dt = New DataTable
        sql = "select y.id,y.item_name from TB_COUNTER_QUEUE x left join TB_ITEM y on x.item_id = y.id where DATEDIFF(D,GETDATE(),service_date) = 0 and status in (1,2,4,6,8) and item_id = " & myUser.skill_id & " and queue_no = '" & FixDB(QueueNo) & "' and customer_id = '" & FixDB(CustomerID) & "' union all select y.id,y.item_name from TB_COUNTER_QUEUE x left join TB_ITEM y on x.item_id = y.id where DATEDIFF(D,GETDATE(),service_date) = 0 and status in (1,2,4,6,8) and item_id <> " & myUser.skill_id & " and queue_no = '" & FixDB(QueueNo) & "' and customer_id = '" & FixDB(CustomerID) & "'"
        dt = getDataTable(Sql)
